<script lang="ts">
  ...
  let showingFilenames = $state(false);
  let showingSettings = $state(false);
  let showingRecentlyDeleted = $state(false);

  let viewingDeletedFile = $state(false);

  // DOM elements
  let editorElement = $state<HTMLElement>() as HTMLElement;
  let searchInNoteElement: FindAndReplace =
    $state<FindAndReplace>() as FindAndReplace;
  ...
  async function initNotes() {
    try {
      ...
      const notesDirExists = await exists("notes", {
  ...
  async function initRecentlyDeleted() {
    try {
      const notesDirExists = await exists("recently-deleted", {
        baseDir: BaseDirectory.AppData,
      });
      if (!notesDirExists) {
        await mkdir("recently-deleted", {
          baseDir: BaseDirectory.AppData,
          recursive: true,
        });
      } 
  ...
  async function _loadNotes(loadDeletedNotes = false): Promise<Note[]> {
    const data: any = await invoke("read_notes_dir", {
      recentlyDeleted: loadDeletedNotes,
    });
  ...
  function _searchResultAsTokensV2(
    line: string,
    searchStrLowercase: string,
    searchForStrInLine: boolean = true,
  ) {
    let idx = searchForStrInLine
      ? line.toLowerCase().indexOf(searchStrLowercase)
      : -1;
    if (idx == -1) {
    ...
    return [
      ...
      {
        highlight: true,
        text: line.substring(idx, idx + searchStrLowercase.length),
      },
      {
        highlight: false,
        text: line.substring(idx + searchStrLowercase.length),
      },
    ];
  }
  ...
  async function onNoteClick(
    ...
    try {
      ...
      viewingDeletedFile = false; // Just in case, always revert to false
      throw err;
    }
    viewingDeletedFile = isDeletedNote; // Set state if viewing a deleted file
  ...
  async function onRestoreNoteClick() {
    ...
    if (noteIndex >= 0) {
      ...
      if (success) {
        notes.unshift(note);
        notes = notes;
        await onNoteClick(notes[0], { saveOnExit: false, isDeletedNote: false });
        viewingDeletedFile = false;
  ...
  async function deleteRecentlyDeletedNote(filename) {
    const index = deletedNotes.findIndex((x) => x.filename == filename); // should be first note, most of the time
    if (index < 0) {
      console.error(`${filename} not in notes list?`);
      return;
    }
    ...
    if (deletedNotes.length > index && deletedNotes[index]) {
      // Go to next deleted note in the list
      await onNoteClick(deletedNotes[index], { saveOnExit: false, isDeletedNote: true });
    } else if (deletedNotes.length > 0) {
      // Last note was deleted; open first note.
      await onNoteClick(deletedNotes[0], { saveOnExit: false, isDeletedNote: true });
    } else {
      // No more deleted notes
      toggleUndelete();
    }
  }
  ...
  function updateMatchingNoteMeta(filename: string, first2Lines: string[]) {
    const matchingNoteIdx = matchingNotes.findIndex(
    ...
    if (!newTitle) {
      ...
      matchingNotes[0].note_meta.search_title_as_tokens = [
        { highlight: false, text: "Untitled" },
      ];
      ...
      matchingNotes[0].note_meta.search_title_as_tokens =
        _searchResultAsTokensV2(newTitle, searchString.toLowerCase());
    }
    // TODO: also update subtitle, like the title above

    matchingNotes[0].modified = Date.now();
    matchingNotes[0].note_meta.modifiedTime = _getModifiedAtStr(
      notes[0].modified,
    );
  }
  ...
  function _doSearch() {
    ...
    invoke("search_handler", { searchString: searchStringLocked }).then(
      (data: any) => {
        ...
        if (data && data.data) {
          ...
          for (let note of notes) {
            ...
            if (match) {
              note.note_meta.search_title_as_tokens = _searchResultAsTokensV2(
                match.first_line,
                searchStrLowercase,
                match.first_line_matches,
              );

              const second_line = isWhitespace(match.second_line)
                ? ""
                : "..." + match.second_line;
              note.note_meta.search_subtitle_as_tokens =
                _searchResultAsTokensV2(
                  second_line,
                  searchStrLowercase,
                  match.second_line_matches,
                );
  ...
  function toggleUndelete() {
    ...
    if (showingRecentlyDeleted) {
      ...
      viewingDeletedFile = false;
      if (notes?.length) {
        onNoteClick(notes[0], { saveOnExit: false, isDeletedNote: false });
      } else {
  ...
  function onContextMenu(event: MouseEvent, note: Note) {
    ...
    myContextMenu.popup(
      new LogicalPosition({ x: event.clientX, y: event.clientY }),
    );
...
<main bind:this={mainElement}>
  ...
  >
    ...
    <div slot="secondary">
      ...
      <div class="header header-right">
          ...
          <div class="center-items">
            <div id="toolbar">
              <span class="ql-formats">
                <FormatDropdown {editor}></FormatDropdown>
              </span>
              <span class="ql-formats">
                <button
                  data-ql-format="ql-code-block"
                  class="custom-icon-btn toolbar-button"
                >
        ...
        <div class:hidden={!(showingRecentlyDeleted && viewingDeletedFile)} class="restore-delete-toolbar">
          <div>
            <span style="margin-left: 8px; margin-right: 12px; color: var(--text-color-secondary)">Viewing deleted note</span>
            <button class="regular" onclick={onRestoreNoteClick}>
              Restore
            </button>
            <button class="regular" onclick={onDeleteNoteClick}>
              Delete
            </button>
          </div>
        </div>
  ...
  <CornerButtonGroup
    style="position: fixed; bottom: 12px; left: 16px; z-index: 99;"
    onSettingsClick={() => {
      showingSettings = !showingSettings;
    }}
    onUndeleteClick={toggleUndelete}
    recentlyDeletedIsOpen={showingRecentlyDeleted}
  ></CornerButtonGroup>
</main>
